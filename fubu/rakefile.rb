...
task :compileAsp => [:clean, :version] do
  ...
  AspNetCompilerRunner.compile :webPhysDir => "src/__NAME__.Web", :webVirDir => "localhost/xyzzyplugh"
   
  
  copyOutputFiles "src/__NAME__.Web/bin/", "*.*", props[:archive]
  copyOutputFiles "src/__NAME__.Web/Views/", "*.*", props[:archive]
...
task :compileSln do
	MSBuildRunner.compile :compilemode => COMPILE_TARGET, :solutionfile => 'src/__NAME__.sln', :clrversion => CLR_VERSION
...
task :specs => :compileSln do
  ...
  runner.executeTests ['__NAME__.MSpec.Tests']
end
